  ...
  return (
    <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
      ...
      {segs.map((s, i) => {
        ...
        const el = <circle key={i} cx={size/2} cy={size/2} r={r} fill="none" stroke={s.color} strokeWidth={stroke} strokeDasharray={`${dash} ${c - dash}`} strokeDashoffset={-off} />;
        off += dash;
        return el;
      })}
    </svg>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    (async () => {
      setALoading(true);
      try { setRawEvents(await fetchAnalytics()); }
      catch(err){ console.error(err); setRawEvents([]); }
  ...
  const contentTotal = data
    ? data.projects + data.skills + data.experiences + data.education + data.certifications : 0;
        ...
        <StatsGrid>
          {cards.map((c, i) => (
            <StatCard key={c.key} $a={c.accent} $d={`${i * 0.06}s`}>
              ...
              {loading ? <Skel h="44px" w="50%" /> : <StatVal $d={`${i * 0.06 + 0.1}s`}>{data?.[c.key] ?? 0}</StatVal>}
