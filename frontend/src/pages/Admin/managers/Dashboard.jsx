...
const BarWrap = styled.div`
  display: flex; align-items: flex-end; gap: 7px; height: 120px;
`;
...
const BarInner = styled.div`
  ...
  &::after {
    ...
    height: 30%; background: rgba(255,255,255,0.06);
    border-radius: 4px 4px 0 0;
  }
`;
...
const MetricVal = styled.span`
  font-size: 1.1rem; font-weight: 800; color: ${({ $c }) => $c || 'white'};
`;

// ── Donut ──────────────────────────────────────────────────────
const DonutWrap = styled.div`
  position: relative; width: 140px; height: 140px; margin: 10px auto 18px;
`;
...
const LegDot = styled.div`
  width: 7px; height: 7px; border-radius: 50%; background: ${({ $c }) => $c};
`;
...
const LegPct = styled.span`
  font-family: 'JetBrains Mono', monospace;
  font-size: 0.68rem; font-weight: 700; color: ${({ $c }) => $c};
`;
...
const FeedText = styled.div`
  font-family: 'JetBrains Mono', monospace;
  font-size: 0.64rem; color: rgba(255,255,255,0.28); margin-top: 1px;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
`;

const FeedMeta = styled.div`
  font-family: 'JetBrains Mono', monospace;
  font-size: 0.58rem; color: rgba(255,255,255,0.15); margin-top: 1px;
`;
...
// ── Stat card config (6 cards, 3-col grid) ─────────────────────
const CARDS = [
...
const Dashboard = () => {
  ...
  // Donut: content items only (excluding messages)
  ...
  // Merge dynamic data into card config
  const cards = CARDS.map(c => ({
    ...c,
    delta: c.key === 'messages'
      ? (data ? `${data.unread} unread` : '—')
      : c.delta,
    pct: c.key === 'messages'
      ? (data ? Math.round((data.unread / Math.max(data.messages, 1)) * 100) : 0)
      : c.pct,
  }));
    ...
    <>
      ...
      <Wrap>
        ...
        <MidRow>
          ...
          {/* Donut — now shows 5 content types */}
          <Panel $d="0.2s">
