...
import { useSendMessage } from '../../Services/FetchData';
import { sendMessage, getAboutMe } from "../../Services/Api";
...
const defaultContactItems = [
  { icon: '✉', label: 'Email',    value: 'contact@example.com', href: 'mailto:contact@example.com', color: '#22d3ee' },
  { icon: '📍', label: 'Location', value: 'Enter your location', href: '#',                    color: '#7b2fff' },
];
    ...
    const loadContactData = async () => {
      try {
        ...
        if (aboutData?.contact) {
          ...
          // Build contact items from fetched data
          const items = [];
          ...
          // Build socials from fetched data
          const socialsList = [];
    ...
    <ContactContainer id="contact">
      <Inner>
          ...
          <InfoPanel>
            ...
            <ContactItems>
              {contactItems.map(item => (
                <ContactItem key={item.label} href={item.href} target="_blank" rel="noopener noreferrer">
            ...
            <div>
              ...
              <SocialRow>
                {socials.map(s => (
                  <SocialBtn key={s.label} href={s.href} color={s.color} target="_blank" rel="noopener noreferrer">
                    <span>{s.icon}</span>
                    <span>{s.label}</span>
                  </SocialBtn>
                ))}
        ...
        <FooterStrip>
          ...
          <FooterCopy>© 2026 Darshan Walhe. All rights reserved.</FooterCopy>
          <FooterLinks>
